...
let ADD_LT_FLOATS = argv.addparams;
...
if (argv.parameters === '') {
  ...
  let json = JSON.parse(fs.readFileSync(__dirname + '/../metadata/SSCWeb/SSCWeb-catalog.json', 'utf8'));
...
function addParameters(recordArray) {
  ...
  for (i of addParameters.ltIndices) {
    ...
    let LTFloat = parseFloat(LTArray[0]) 
                 + parseFloat(LTArray[1])/60.0
                 + parseFloat(LTArray[2]/3600.0);
    if (DEBUG) {
      LTArray.push(LTFloat.toFixed(16));
      console.log(LTArray);
    }
    recordArray.push(LTFloat.toFixed(16));
  }
  return recordArray;
}
