...
function initServer (CATALOGS, PREFIXES, app, argv) {
  ...
  const options = serverOptions(argv)
      ...
      require(protocol)
        ...
        .listen(port, function () {
          ...
          if (argv.verify) {
            ...
            const serverConfig = metadata(PREFIXES[0], 'server')
            // verify() exits with code 1 if error.
            const vopts = { url: url + '/' + PREFIXES[0] + '/hapi', output: 'console' }
  ...
  function serverOptions (argv) {
    ...
    if (argv.https === false) {
      ...
      if (argv.cert && argv.key) {
        ...
        const com = 'sh \"' + __dirname + '/../ssl/gen_ssl.sh' + '\"'
