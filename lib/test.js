...
function commands(commandarr, testName, exit_if_fail) {
  ...
  for (var i = 0;i < commandarr.length;i++) {
    ...
    let opts = {"return": "object", "exit_if_fail": exit_if_fail, "logerror": false};
    ...
    let result = testoutput(commandarr[i], child);
...
function testoutput(testobj, child) {
  ...
  if ("status" in testobj) {
    retobj = compare(child.status,testobj.status,'status');
    results.push(retobj);
  }
