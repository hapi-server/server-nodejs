...
if (argv.columns !== '') {
  ...
  const tmp = argv.columns.split(',')
  for (i = 0; i < tmp.length; i++) {
    if (/-/.test(tmp[i])) {
      tmp2 = tmp[i].split('-')
      const expanded = range(parseInt(tmp2[0]) - 1, parseInt(tmp2[1]) - 1)
...
function readlines (readableStream) {
  const rl = readline.createInterface({
    ...
    .on('line', function (line) {
      ...
      let time = linea[0].trim().replace(/Z$/, '')
      const l = time.length
      let start = argv.start.replace(/Z$/, '')
      let stop = argv.stop.replace(/Z$/, '')
      const m = Math.min(l, start.length, stop.length)
      start = start.slice(0, m)
      stop = stop.slice(0, m)
      time = time.slice(0, m)
