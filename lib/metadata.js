...
function prepmetadata(FILES, FORCE_START, SKIP_CHECKS, cb) {
  ...
  landing(json, serverfile, catalogid, FORCE_START);
  ...
  let formats = json.data.formats || "csv";
  if (!json.data.file && !json.data.command && !json.data.url) {
    let msg = "A 'file', 'command', or 'url' must be specified in the 'data' node.";
    log.error(msg,true);
  }
  ...
  updatemetadata(true, FORCE_START, SKIP_CHECKS, json, (err) => {
    if (json.data.testcommands !== undefined && SKIP_CHECKS == false) {
      let Nc = json.data.testcommands.length;
      let plural = Nc > 1 ? "s" : "";
...
function updatemetadata(exit_if_fail, FORCE_START, SKIP_CHECKS, json, cb) {
  ...
  getmetadata(json, "catalog", exit_if_fail, (e, xcatalog) => {
    if (e) {
      log.error(e, exit_if_fail);
      if (!exit_if_fail) cb(e);
    } else {
  ...
  function createcat(ds_num, xcatalog) {
    ...
    function checkinfo(info) {
      ...
      for (let infoType of ["info", "info_inline", "info_url", "info_file", "info_command"]) {
...
function getmetadata(obj, nodeType, exit_if_fail, cb) {
  ...
  function getinfos(caturl, catalog, cb) {
    ...
    function getone(i) {
      ...
      let url = caturl.replace("hapi/catalog","hapi/info") + "?id=" + catalog[i].id;
      request(url, (e, data) => {
        if (e) {
          cb(e);
        } else {
          try {
            ...
            log.error(url + " output is not JSON parse-able. Try https://jsonlint.com/.");
            cb(e);
          }
          getone(++i);
        }
  ...
  function readfile(fname, cb) {
    ...
    fs.readFile(fname, "utf8", (e, info) => {
      if (e) {
        log.error("Could not read " + fname + ". Error message: " + e.message, exit_if_fail);
        if (!exit_if_fail) cb(e);
      }
      try {
        info = JSON.parse(info);
      } catch (e) {
        log.error(fname + " is not JSON.parse-able. Try https://jsonlint.com/.", exit_if_fail);
        if (!exit_if_fail) cb(e);
      }
      cb(null, info);
    });
  ...
  if (type === undefined) {
    let msg = `No '${nodeType}_{file,url,command,inline}', or '${nodeType}' node found.`;
    log.error(msg,true);
  }
  ...
  if (type === "url" || (nodeType == null && nodeStr.startsWith('http'))) {
    ...
    request(nodeStr, (e, data) => {
      if (e) {
        log.error("Error getting " + nodeStr + ": " + e.message, exit_if_fail);
        if (!exit_if_fail) cb(e);
      }
      ...
      try {
        ...
        log.error(nodeStr, exit_if_fail);
        if (!exit_if_fail) cb(e);
      }
      if (nodeType === "catalog" && /hapi\/catalog\/?$/.test(nodeStr)) {
        ...
        getinfos(nodeStr, obj.catalog, (e, obj) => {
          if (e) {
            log.error(e.message, exit_if_fail);
            if (!exit_if_fail) cb(e);
          } else {
            cb(null, obj);
          }
    ...
    log.info("Executing: " + nodeStr);
    let eopts = {"cwd": __dirname + "/..", "maxBuffer": 100000000};
    require('child_process').exec(nodeStr, eopts, (e, info) => {
      if (e) {
        ...
        try {
          ...
          log.error(nodeStr + " output is not JSON parse-able. Try https://jsonlint.com/. Exiting.",exit_if_fail);
          if (!exit_if_fail) cb(e);
        }
        cb(null, info);
      }
...
function readCatalogConfig(file) {
  ...
  try {
    ...
    let msg = file + " is not JSON parse-able. Try https://jsonlint.com/. Message: " + ex.message;
    log.error(msg,true);
  }
  ...
  if (!json["data"]) {
    let msg = file + " must have a 'data' object";
    log.error(file + msg, true);
  }
  if (!json["data"]["contact"]) {
    let msg = file + " must have a non-empty string value in data.contact";
    log.error(file + msg, true);
  }
...
function landing(json, serverfile, catalogid, FORCE_START) {
  ...
  if (!fs.existsSync(landingFile) || !fs.lstatSync(landingFile).isFile()) {
    log.error(`landingFile = '${landingFile}' not found`, true);
  }

  if (!fs.existsSync(landingPath) || !fs.lstatSync(landingPath).isDirectory()) {
    log.error(`landingPath = '${landingPath}' not found`, true);
  }
...
function jsonWarn(formats, catalogid) {
  if (!formats.includes("json")) {
    ...
    for  (let i in metadata.cache[catalogid]['info']) {
