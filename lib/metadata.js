...
//let fullPath = fs.realpathSync('./node_modules/hapi-server-verifier');
//const is = require(fullPath + "/is.js");
...
function ishapi(json, schema, subschema, force, dsid) {
	...
	if (v.error) {
		...
		if (force) { 
			log.info(clc.yellow.bold("Starting server with invalid HAPI because --ignore command-line option used."));
		} else {
			log.info(clc.yellow.bold("Use the --ignore command-line option to force start the server when metadata is not schema-valid. Exiting."));
			process.exit(1);
		}
	}
}
...
function configVars(configfile) {
	...
	for (key in config) { 
		if (config[key] !== "" && config[key].slice(0,2) !== "__") {
			log.debug("Found " + key + " in " + configfile + ". Using it.");
			configd[key] = config[key]; // Update default
		} else {
			log.debug(key + " not given in " + configfile + ".");
		}
	}

	// Override defaults and config file with shell environment variables
	if (env.HAPISERVERPATH) {
		log.debug(ds() + "Found HAPISERVERPATH environment variable. Using it.");
	...
	if (env.NODEEXE) {
		log.debug("Found NODEEXE environment variable. Using it.");
		configd["NODEEXE"] = env.NODEEXE;
	} else {
		log.debug("NODEEXE environment variable not found.");			
	}
	if (env.PYTHONEXE) {
		log.debug("Found PYTHONEXE environment variable. Using it.");
		configd["PYTHONEXE"] = env.PYTHONEXE;
	} else {
		log.debug("PYTHONEXE environment variable not found.");			
	}

	if (configd["PYTHONEXE"] === "") {
		log.debug("Working python executable not found.");
	}
...
function getmetadata(str, type, exit_if_fail, cb) {
	...
	if ( (type === "url") || (type === "catalog" && str.substring(0, 4) === 'http') ) {
		request(str, (e, data) => {
			if (e) {
				console.log(clc.red("Error getting " + str + ": " + e.message));
			...
			try {
				...
				console.log(clc.red(str + " output is not JSON parse-able. Try https://jsonlint.com/. Exiting."));
				exit_conditionally(exit_if_fail, e, cb);
			}
			if (/hapi\/catalog\/?$/.test(str)) {
				log.info("Catalog node in server configuration is a HAPI server. Getting info metadata for all server datasets.");
		...
		if( (type === "file") || (type === "catalog" && fileExistsSync(str.split(" ")[0])) ) {
			log.info("It is not a command line command. Will treat as a file and attempt to read as JSON.");
			// Read info files
...
function updatemetadata(exit_if_fail, FORCE_START, SKIP_CHECKS, json, cb) {
	...
	metadata.newcache = JSON.parse(JSON.stringify(metadata.cache));
	...
	if ( json.catalog_file || json.catalog_url || json.catalog_command ) {
		...
		if ( json.catalog || json.catalog_inline ){
			if (json.catalog) {
				console.warn("Warning: catalog is deprecated. Use 'catalog_file', 'catalog_command' or 'catalog_url'.");
				if (typeof(json.catalog) === 'string') {
					...
					log.info(clc.blue("Catalog is not a string."))
	...
	function createcat(ds_num, xcatalog) {
		...
		if (typeof(xcatalog[ds_num].info) === 'string') {
			// Resolve info node.
			// TODO: Should have info_file, info_command, and info_url
			...
			// JSON
			checkinfo(xcatalog[ds_num].info);
		}
		...
		function checkinfo(info) {
			...
			if (SKIP_CHECKS == false) {
				...
				ishapi(info_resolved, json["HAPI"], 'info', FORCE_START, xcatalog[ds_num]['id']);
				//log.info("Checked schema validity of /info response metadata for " + id);
			}
			metadata.newcache[catalogid]['info'][id] = info;
			metadata.newcache[catalogid]['info-raw'][id] = info_resolved;
		...
		function checkcat() {
			...
			metadata.cache = Object.assign({}, metadata.newcache);
			metadata.newcache = {};
			cb();
		}
	}
...
function prepmetadata(FILES, FORCE_START, SKIP_CHECKS, cb) {	
	...
	if (!prepmetadata.Nservers) {
		...
		log.info(clc.green("-".repeat(msg.length)));
		log.info(clc.green(msg));
	}
	...
	function readandparse(file) {
		if (!fs.existsSync(file)) {
			console.error(ds() + clc.red("Did not find " + file + ". Exiting."));
			process.exit(1);
		}
		log.info("Start reading, parsing, and resolving " + file);
		try {
			var str = fs.readFileSync(file);
		} catch (ex) {
			console.error(ds() + clc.red(file + " is not readable. Exiting."));
			console.error(ds() + clc.red(ex.message));
			process.exit(1);		
		}
		try {
	...
	if (!json.server) {
		console.error(ds() + clc.red(serverfile + " does not have a server node."));
		process.exit(1);		
	}
	if (!json.server.contact) {
			console.error(ds() + clc.red(serverfile + " must have a non-empty contact string in the server object."));
			process.exit(1);
	}
	if (!json.data.contact) {
			console.error(ds() + clc.red(serverfile + " must have a non-empty contact string in the data object."));
			process.exit(1);
	}
	...
	if (landingFile === "" && landingPath === "") {
		log.info("No landingFile or landingPath in " + serverfile.replace(ROOT, "") 
	...
	if (landingFile !== "") {
		if (!fs.existsSync(landingFile)) {
			...
			if (FORCE_START) {
				...
				log.info(msg + "Will start server with no landing page b/c ignore=true.");
			} else {
				console.error(clc.red(msg + "Exiting b/c ignore=false."));
				process.exit(1)
			}
		} 
	}
	...
	if (!json.data) {
		console.error(clc.red(serverfile + " Does not have a 'data' node. Exiting."));
		process.exit(1);		
	}
	...
	if (!json.data.file && !json.data.command && !json.data.url && !json.data.proxy) {
		let msg = "A 'command', 'file', 'url', or 'url' must be specified in the 'data' node.";
	...
	if ( !json.catalog && !json.catalog_file && !json.catalog_url && !json.catalog_command && !json.catalog_inline ) {
		console.error(clc.red(serverfile + " does not have a 'catalog' object. Exiting."));
		process.exit(1);		
	}
	...
	if (update_seconds) {
		if (!Number.isInteger(update_seconds)) {
			console.error(clc.red(serverfile + " has a non-integer value for server.catalog-update."));
			process.exit(1);			
		}
		if (update_seconds < 0) {
			console.error(clc.red(serverfile + " has a negative value for server.catalog-update."));
			process.exit(1);			
		}
	...
	updatemetadata(true, FORCE_START, SKIP_CHECKS, json, (err) => {
		...
		log.info("Finished reading, parsing, and resolving metadata in " + FILES[0]);
		FILES.shift();
