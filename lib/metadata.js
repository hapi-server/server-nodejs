...
async function prepmetadata(FILES,FORCE_START,VERIFIER,PLOTSERVER,cb) {	
	...
	for (var i = 0;i < json.catalog.length; i++) {
		...
		if (FORCE_START) {
			//console.log(ds() + "Checking /info response metadata for dataset with id = " + id + ".");
		}
	...
	if (FORCE_START) {
		console.log(ds() + "Checking /catalog response metadata.");
	}
