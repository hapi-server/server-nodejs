...
//const verifier = require('./verifier.js').verifier(0);
//module.exports.verifier = verifier(0)
...
function prepmetadata(FILES, FORCE_START, SKIP_CHECKS, cb) {
  ...
  if (!json.data.file && !json.data.command && !json.data.url && !json.data.proxy) {
    let msg = "A 'file', 'command', 'url', or 'proxy' must be specified in the 'data' node.";
...
function updatemetadata(exit_if_fail, FORCE_START, SKIP_CHECKS, json, cb) {
  ...
  if (json.catalog_file || json.catalog_url || json.catalog_command) {
    ...
    if (json.catalog || json.catalog_inline) {
      if (json.catalog) {
        console.warn("Warning: catalog is deprecated. Use 'catalog_{file,command,url,inline}'");
  ...
  function createcat(ds_num, xcatalog) {
    ...
    if (typeof(xcatalog[ds_num].info) === 'string') {
      ...
      getmetadata(xcatalog[ds_num].info, "catalog", exit_if_fail, (e, info) => {
        if (e) {
          conosole.log("Failed to get " + xcatalog[i].info + ".");
          exit_conditionally(exit_if_fail, e, cb)
        } else {
          checkinfo(info);
        }
    ...
    function checkinfo(info) {
      ...
      let info_resolved = resolveJSONRefs(info);
...
function getmetadata(str, type, exit_if_fail, cb) {
  ...
  str = conf.replaceConfigVars(str);
  ...
  if ( (type === "url") || (type === "catalog" && str.substring(0, 4) === 'http') ) {
    ...
    if( (type === "file") || (type === "catalog" && existsSync(str.split(" ")[0])) ) {
...
function readCatalogConfig(file) {
  ...
  if (!json.server) {
    console.error(ds() + clc.red(file + " does not have a server node."));
    process.exit(1);		
  }
  if (!json.server.contact) {
    console.error(ds() + clc.red(file + " must have a non-empty contact string in the server object."));
    process.exit(1);
  }
  if (!json.data.contact) {
    console.error(ds() + clc.red(file + " must have a non-empty contact string in the data object."));
    process.exit(1);
  }
  if (!json.server.HAPI && !json.about && !json.about.HAPI) {
    console.error(ds() + clc.red(file + " must have a non-empty value for HAPI in the server object (or the about object for HAPI 3.0+)"));
    process.exit(1);
  }
  if (json.about) {
  ...
  if (!json.catalog && !json.catalog_file && !json.catalog_url && !json.catalog_command && !json.catalog_inline ) {
    console.error(clc.red(file + " does not have a 'catalog' object. Exiting."));
    process.exit(1);		
  }
  return json;
}
