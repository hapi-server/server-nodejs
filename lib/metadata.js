...
function prepmetadata(catalogfile,HAPIVERSION,FORCE,VERIFIER,PLOTSERVER) {
	...
	if (json.data.command) {
		json.data.command = json.data.command.replace("${cwd}", process.cwd());
		if (json.data.testcommands) {
			var testcommands = json.data.testcommands;
			for (var i = 0;i < testcommands.length;i++) {
				testcommands[i].command = testcommands[i].command.replace("${cwd}", process.cwd());
			}
			json.data.testcommands = testcommands;
