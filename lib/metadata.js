...
function ishapi(json, schema, subschema, force) {
	...
	if (v.error) {
		...
		if (force) { 
			...
			log.info("\n" + clc.yellow("Use the --ignore command-line option to force start the server when metadata is not schema-valid. Exiting."));
			process.exit(1);
		}
	}
}
	...
	if ( (type === "url") || (type === "catalog" && str.substring(0, 4) === 'http') ) {
		...
		if( (type === "file") || (type === "catalog" && fileExistsSync(str.split(" ")[0])) ) {
			...
			if( type === "command" || type === "catalog" ){
				str = str.trim();
				if (type === "catalog") {
					log.info("command is not a file; executing " + str);
				}
...
function updatemetadata(exit_if_fail, FORCE_START, json, cb) {
	...
	function createcat(ds_num, xcatalog) {
		...
		log.info("Working on dataset " + id);
		...
		function checkcat() {
			log.info("Checking /catalog response metadata for " + catalogid);
			...
			metadata.cache = JSON.parse(JSON.stringify(metadata.newcache));
			log.info("Checked /catalog response metadata for " + catalogid);
			cb();
		}
	}
}
...
function prepmetadata(FILES, FORCE_START, VERIFIER, PLOTSERVER, cb) {	
	...
	if (landingFile !== "") {
		if (!fs.existsSync(landingFile)) {
			...
			if (FORCE_START) {
				...
				console.log(clc.red(msg + "Exiting b/c ignore=false."));
				process.exit(1)
			}
		} 
	}
	...
	if (!json.data) {
		console.log(clc.red(serverfile + " Does not have a 'data' node. Exiting."));
		process.exit(1);		
	}
	...
	if ( !json.catalog && !json.catalog_file && !json.catalog_url && !json.catalog_command && !json.catalog_inline ) {
		console.log(clc.red(serverfile + " does not have a 'catalog' object. Exiting."));
		process.exit(1);		
	}
	...
	if (update_seconds) {
		if (!Number.isInteger(update_seconds)) {
			console.log(clc.red(serverfile + " has a non-integer value for server.catalog-update."));
			process.exit(1);			
		}
		if (update_seconds < 0) {
			console.log(clc.red(serverfile + " has a negative value for server.catalog-update."));
			process.exit(1);			
		}
