...
function ishapi(json, schema, subschema, force) {
	...
	if (v.error) {
		...
		process.exit(1);
		/* Can't start without valid HAPI version (version with schema in
		   hapi-server-verifier) b/c timeCheck call to is.HAPITime() will
		   generate error because it cannot find the associated schema object.
		if (force) { 
