...
async function prepmetadata(FILES,HAPIVERSION,FORCE,VERIFIER,PLOTSERVER,cb) {	
	...
	for (var i = 0;i < json.catalog.length; i++) {
		...
		ishapi(info_resolved,SCHEMAVERSION,'info',FORCE);		
	...
	FILES.shift();
	prepmetadata(FILES,HAPIVERSION,FORCE,VERIFIER,PLOTSERVER,cb)
}
