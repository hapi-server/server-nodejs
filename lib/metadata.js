...
function ishapi(json, schema, subschema, force) {
	...
	var s = is.HAPIVersion(json.HAPI);
	if (s.error) {
		console.log(ds() + clc.red("Schema is not available for HAPI " + json.HAPI + ".\n"));
		process.exit(1);
	}
		...
		if (typeof(obj) === "object") {
			for (var prop in obj) {
				if (obj[prop] && typeof(obj[prop]) === "object" && 
					...
					//console.log(hashOfObjects[obj[prop]["$ref"]]);
...
function configVars(configfile) {
	...
	if (isWin) {
		var child = require('child_process')
						.spawnSync('where python',{"shell": true});
		if (child.stdout) {
			configd["PYTHONEXE"] = child.stdout.toString().slice(0,-1);
			if (configd["PYTHONEXE"].includes("Microsoft\\WindowsApps")) {
				configd["PYTHONEXE"] = "";
			}
		}
		if (child.status == 0 && configd["PYTHONEXE"] !== "") {
			//console.log(ds() + "Using system Python (if needed): " + pythonpath);
	...
	if (!configd["PYTHONEXE"]) {
		//console.log(ds() + "Python not found in path. Will use pre-packaged Python (if needed) unless location set in config.json or as env variable.");
	...
	for (key in config) { 
		if (config[key] !== "" && config[key].slice(0,2) !== "__") {
			...
			console.log(ds() + "Setting " + key + " = " + config[key]);
		}
	}

	// Override defaults and config file with shell environment variables
	console.log(ds() + "Reading environment variables.");
	if (env.HAPISERVERPATH) {
		configd["HAPISERVERPATH"] = env.HAPISERVERPATH;
		console.log(ds() + "Setting HAPISERVERPATH to " + env.HAPISERVERPATH);
	}
	if (env.HAPISERVERHOME) {
		configd["HAPISERVERHOME"] = env.HAPISERVERHOME;
		console.log(ds() + "Setting HAPISERVERHOME to " + env.HAPISERVERHOME);
	}
	if (env.NODEEXE) {
		configd["NODEEXE"] = env.NODEEXE;
		console.log(ds() + "Setting NODEEXE to " + env.NODEEXE);
	}
	if (env.PYTHONEXE) {
		configd["PYTHONEXE"] = env.PYTHONEXE;
		console.log(ds() + "Setting PYTHONEXE to " + env.PYTHONEXE);
	}
...
function updatemetadata(exit_if_fail, FORCE_START, json, cb) {
	...
	function createcat(ds_num, xcatalog) {
		...
		function checkinfo(info, cb) {
			...
			//console.log(info.parameters[1]);
			let info_resolved = resolve(info);
			//console.log(info_resolved.parameters[1]);
			//process.exit(0);
