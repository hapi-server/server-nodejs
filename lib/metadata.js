...
function pythonexe() {
	...
	if (isWin) {
		...
		if (child.status == 0 && PYTHONEXE !== "") {
			if (debug) console.log(ds() + "Using system Python (if needed): " + pythonpath);
			PYTHONEXE = "python";
		}
	} else {
	...
	if (PYTHONEXE === "") {
		if (debug) console.log(ds() + "Python not found in path. Will use pre-packaged Python (if needed) unless location set in config.json or as env variable.");
