...
function updatemetadata(exit_if_fail, FORCE_START, json, cb) {
	...
	function createcat(dsid, xcatalog) {
		// Each element of xcatalog is a dataset

		// Resolve info nodes and checks catalog and info.

		let id = xcatalog[dsid].id;

		metadata.cache[catalogid]['ids'].push(id);

		catalog.catalog[dsid] = xcatalog[dsid];
		if (typeof(xcatalog[dsid].info) === 'string') {
			// Resolve info node.
			getmetadata(xcatalog[dsid].info, exit_if_fail, (e, info) => {
				if (e) {
			...
			checkinfo(xcatalog[dsid].info, checkcat);
		}
		...
		function checkinfo(info, cb) {
			...
			delete catalog.catalog[dsid].info;
