...
function pythonexe() {
	...
	if (PYTHONEXE === "") {
		...
		if (debug) console.log(ds() + "Python not found in path. Will use " + PYTHONEXE + " if available/needed.");
...
function updatemetadata(exit_if_fail, FORCE_START, json, cb) {
	...
	function createcat(ds_num, xcatalog) {
		...
		console.log(ds() + "Working on dataset " + id);
...
function prepmetadata(FILES, FORCE_START, VERIFIER, PLOTSERVER, cb) {	
	...
	if (!prepmetadata.Nservers) {
		...
		console.log(ds() + clc.green("-".repeat(msg.length)));
		console.log(ds() + clc.green(msg));
	}
	...
	function readandparse(file) {
		...
		console.log(ds() + "Start reading, parsing, and resolving " + file);
		try {
