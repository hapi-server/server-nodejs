...
4. Installation and Configuration
	...
	1. [Verifier Configuration](#Verifier)
	1. [Plot Server Configuration](#Plot_Server)
	1. [Apache and Nginx Configuration](#Apache_and_Nginx)
5. [Metadata](#Metadata)
6. [Tests](#Tests)
7. [Contact](#Contact)
...
The following examples are included in the [metadata](https://github.com/hapi-server/server-nodejs/blob/master/metadata/) directory.
...
```bash
node server.js --catalog Example0
```

and then open [http://localhost:8999/Example1/hapi](http://localhost:8999/Example0/hapi). You should see the same landing page as that at [http://hapi-server.org/servers/Example0/hapi](http://hapi-server.org/servers/Example1/hapi).
...
```
node server.js --catalog QinDenton
```
...
```bash
node server.js --catalog AutoplotExample1
```
...
```bash
node server.js --catalog AutoplotExample2
```
...
node server.js
	...
	--prefixes PREFIXES		A comma-separated list of prefixes; default is CATALOGS
	--verifier URL			URL to a verifier that can read from server
	--force					Start if catalog does not validate
	--test					Exit with code 0 or 1 after running tests
...
Data is served from `http://localhost:PORT/PREFIX/hapi` using datasets and command line program template specified in `./metadata/CATALOG.json`. If `public/CATALOG.{htm,html}` is found, it is used as the landing page instead of `public/default.htm`. If `PREFIX` is not given, `PREFIX=CATALOG` is assumed.
...
If `--force` is used, the server will start even if the HAPI metadata node in `metadata/CATALOG.json` is not [HAPI schema](https://github.com/hapi-server/verifier-nodejs/tree/master/schemas) valid.
...
```bash
node server.js --catalogs TestData,Example1
```
...
```bash
node server.js --catalogs TestData,Example1 --prefixes xTestDataS,xExample1
```

will serve the two datasets at

```
http://localhost:8999/xTestData/hapi
http://localhost:8999/xExample1/hapi
```

This could have also been effected by creating symlinks pointing `metadata/xTestData` to `metadata/TestData` and `metadata/xExample1` to `metadata/Example1`.
...
# In another terminal window, start the verifier server
node verify.js --port 9000
```

<a name="Plot_Server"></a>
### 4.3 Plot Server Configuration
...
Find location where `hapiclient` was installed

```
pip show hapiclient | grep Location
# Should see "Location: PATH"
```

Start the plot server

```
cd PATH/hapiclient/plotserver
python hapiplotserver.py --port 5000
```

Start the server and specify the plot server location

```
node server.js --plotserver 'http://localhost:5000/'
```


<a name="Apache_and_Nginx"></a>
### 4.4 Apache and Nginx Configuration
